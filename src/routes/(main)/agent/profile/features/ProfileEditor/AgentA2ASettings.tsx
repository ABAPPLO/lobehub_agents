...
const AgentA2ASettings = memo(() => {
  ...
  const agentCardUrl = useMemo(
    () =>
      activeAgentId ? `${serverOrigin}/a2a/agent/${activeAgentId}/.well-known/agent.json` : '',
    [activeAgentId, serverOrigin],
  );

  const endpointUrl = useMemo(
    () => (activeAgentId ? `${serverOrigin}/a2a/agent/${activeAgentId}` : ''),
    [activeAgentId, serverOrigin],
  );
  ...
  const handleToggle = useCallback(
    async (checked: boolean) => {
      await updateConfig({
        agencyConfig: { ...agencyConfig, a2a: { ...a2aConfig, enabled: checked } },
      });
    },
    [updateConfig, agencyConfig, a2aConfig],
  );
  ...
  const persistRelayField = useCallback(
    (field: string, value: string) => {
      updateConfig({
        agencyConfig: {
          ...agencyConfig,
          a2a: { ...a2aConfig, enabled, relay: { ...relayConfig, [field]: value } },
        },
      });
    },
    [updateConfig, agencyConfig, a2aConfig, enabled, relayConfig],
  );
  ...
  const handleRelayStreamingChange = useCallback(
    async (checked: boolean) => {
      await updateConfig({
        agencyConfig: {
          ...agencyConfig,
          a2a: { ...a2aConfig, enabled, relay: { ...relayConfig, streamingEnabled: checked } },
        },
      });
    },
    [updateConfig, agencyConfig, a2aConfig, enabled, relayConfig],
  ...
  const handleRelayConnect = useCallback(async () => {
    ...
    try {
      const result = await lambdaClient.aiAgent.agentA2ARelayStart.mutate({
  ...
  }, [activeAgentId, updateConfig, agencyConfig, a2aConfig, enabled, relayConfig, t]);

  const handleRelayDisconnect = useCallback(async () => {
    if (!activeAgentId) return;
    setRelayLoading(true);
    try {
      await lambdaClient.aiAgent.agentA2ARelayStop.mutate({ agentId: activeAgentId });
