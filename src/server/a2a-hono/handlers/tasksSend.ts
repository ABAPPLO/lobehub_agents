...
export async function tasksSend(c: Context, request: A2AJsonRpcRequest): Promise<Response> {
  ...
  // Get group detail to find supervisor agent ID
  ...
  try {
    ...
    const result = await aiAgentService.execGroupAgent({
      ...
      groupId,
      message: textContent,
      newTopic: { title: `A2A Task: ${textContent.slice(0, 50)}` },
    });
    ...
    // Store A2A task ID in operation metadata (merge into existing metadata)
    await db
